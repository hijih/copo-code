...
from verl.trainer.ppo.core_algos import agg_loss, compute_policy_loss, kl_penalty, compute_policy_loss_with_global
...
class DataParallelPPOActor(BasePPOActor):
    ...
    @GPUMemoryLogger(role="dp actor", logger=logger)
    def update_policy(self, data: DataProto, global_flg: bool = True):
        ...
        if global_flg != 'no global':
            select_keys.append('global_adv')
            select_keys.append('reward_entropy')
            select_keys.append('reward_entropy')
            select_keys.append('local_reward')
        batch = data.select(batch_keys=select_keys).batch
        has_multi_modal_inputs = "multi_modal_inputs" in data.non_tensor_batch.keys()
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    local_reward = data['local_reward']
                    reward_entropy = data['reward_entropy']
                    ...
                    if entropy_coeff != 0:
                        ...
                        # compute policy loss
                        policy_loss = pg_loss_with_global - entropy_loss * entropy_coeff
                    else:
                        policy_loss = pg_loss_with_global
                    ...
                    data = {
                        ...
                        'actor/pg_losses/max': pg_losses.max().detach().item(),     
                        'actor/global_pg_losses/mean': global_pg_losses.mean().detach().item(),
                        'actor/global_pg_losses/min': global_pg_losses.min().detach().item(),
                        'actor/global_pg_losses/max': global_pg_losses.max().detach().item(),                        
                        'actor/global_loss_weight/mean': global_loss_weight.mean().detach().item(),
                        'actor/global_loss_weight/min': global_loss_weight.min().detach().item(),
                        'actor/global_loss_weight/max': global_loss_weight.max().detach().item(),
                        'actor/pg_loss_with_global': pg_loss_with_global.detach().item(),
